...
func BenchmarkBufio(b *testing.B) {
	...
	enc := json.New("1.2.3", json.Config{Pretty: false, EscapeHTML: false, LocalTime: true})
...
func TestTcpOutput(t *testing.T) {
	var tests = []struct {
	...
	}{
		{
			...
			json.New("1.2.3", json.Config{Pretty: false, EscapeHTML: false, LocalTime: true}),
			[]beat.Event{
				{Fields: event("field", "value")},
			},
...
func run(config Config, codec codec.Codec, batches ...publisher.Batch) (string, error) {
	...
	return withTcpOut(ln, func() {
		for _, b := range batches {
			t.Publish(context.Background(), b)
		}
		t.Close()
	})
}
