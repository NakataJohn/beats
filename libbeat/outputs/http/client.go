...
var (
	logger = logp.NewLogger("output_http_conn")
...
type Client struct {
	...
	params           map[string]string // additional configs
...
// clientSettings struct,it contains the settings for a client.
...
// Connection struct is the settings needed for a Connection.
...
func (client *Client) publishEvents(ctx context.Context, data []publisher.Event) ([]publisher.Event, error) {
	...
	if client.batchPublish {
		...
		client.logger.Debugf("Publishing events in batch.")
		sendErr = client.BatchPublishEvent(data)
		if sendErr != nil {
			return data, sendErr
		}
	} else {
		client.logger.Debugf("Publishing events one by one.")
	...
	client.logger.Debugf("PublishEvents: %d metrics have been published over HTTP in %v.", len(data), time.Since(begin))
